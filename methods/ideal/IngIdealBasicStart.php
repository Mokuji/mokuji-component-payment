...
define('WHOAMI', 'Payment::IngIdealBasicStart');

//Override paths.
$url_path = str_replace('/mokuji/components/payment/methods/ideal/IngIdealBasicStart.php', '', $_SERVER['PHP_SELF']);
...
try{
  $form = $handler->generate_to_ing_form(mk('Data')->get->as_array());
}catch(\Exception $ex){
  mk('Logging')->log('Payment', 'IngIdealBasicStart', 'Failed to start: '.$ex->getMessage());
  mk('Logging')->log('Payment', 'IngIdealBasicStart', mk('Data')->get->dump());
  die($ex->getMessage());
}
